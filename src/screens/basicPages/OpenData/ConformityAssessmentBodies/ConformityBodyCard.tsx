...
export default function ConformityBodyCard({item}: {item: any}) {
  ...
  return (
    <View style={styles.card}>
      <Text h2 bold>
        {isArabic()
          ? item.AccreditationSchemeEnglish
          : item.AccreditationSchemeArabic}
      </Text>
      <View style={[styles.row, {marginTop: 2 * BW()}]}>
        <Text h4 medium>
          {item.AddressEnglish?.trim?.()}
        </Text>
      </View>
      <DashedLine />
      ...
      <View style={styles.row}>
        ...
        <Text h4 medium>
          {item.DateOfInitialAccreditation?.split('T')[0]}
        </Text>
      </View>
      <DashedLine />
