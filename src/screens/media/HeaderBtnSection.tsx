...
export default function HeaderBtnSection(props: any): JSX.Element {
  ...
  const {activeBtn, setActiveBtn, setMediaType} = props;
  const {mediaCat, isLoading}: any = useAppSelector(
  ...
  return (
    <View style={[style.btnHeaderContainer, style.row]}>
      <FlatListComp
        horizontal
        scrollview
        data={mediaCat?.value?.slice().reverse()}
        contentContainerStyle={{flex: 1, alignItems: 'center'}}
        renderItem={({item, index}: any) => (
          <Fragment key={index}>
            <View style={{flex: 1, minHeight: 45 * BW()}}>
              <Button
                style={{...style.btn, minHeight: 45 * BW()}}
                h3
                medium
                title={t(`${item?.FieldValues?.Title}`)}
                styleText={style.title}
                onPress={() => {
                  setActiveBtn(index);
                  setMediaType(item.Id);
                }}
              />
              <Animatable.View
                delay={100}
            ...
            {index != mediaCat.length - 1 && <Line style={style.line} />}
          </Fragment>
        )}
      />
    </View>
