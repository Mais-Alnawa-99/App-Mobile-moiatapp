...
import {transformDutyExemptionData} from './SubmitData';
...
import {mapDutyExemption} from './DataMapper';
...
export default function DutyExemption(props: any): JSX.Element {
  ...
  const _getApplicationDetails = async () => {
    ...
    if (data?.payload.Data?.success) {
      ...
      const newMappedData = mapDutyExemption(updatedData);
  ...
  const onSubmit = (values: any) => {
    try {
      ...
      const body = transformDutyExemptionData({
        ...values,
        userId: userId,
      ...
      dispatch(
        submitDutyExemption({body: {...body, languageId: language}}),
      ).then(result => {
          ...
          return (
            <View>
              ...
              <Input
                ...
                disabled={isDisabled('CustomsNumber')}
              />
              <Input
                textInput
              ...
              <Input
                ...
                disabled={isDisabled('TotalVAlueInAED')}
              />
              <Input
                textInput
              ...
              <StepIndicator
                stepNumber={4}
                style={{marginBottom: 0 * BW(), marginTop: 8 * BW()}}
                stepName={t('IL.DE.ServiceFees')}
              />
              <Input
                textInput
                label={t('IL.DE.ServiceFeePerHSCode')}
                value={values.ServiceFees.toString()}
                disabled
              />
              <Input
                textInput
                label={t('IL.DE.TotalFeeOfApplications')}
