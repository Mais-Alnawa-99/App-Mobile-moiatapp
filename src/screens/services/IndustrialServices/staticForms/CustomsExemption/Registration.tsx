...
import RawMaterialsDetails from './ExemptionRawMaterialDetails';
...
import {mapAddNewRawMaterial} from './DataMapper';
...
interface FormValues {
  ...
  localGulfOriginForeign: string;
  RawMaterials: any[];
  LocalMaterialsCost: any[];
  ForeignMaterialsCost: any[];
  Invoice: any[];
  BillOfLading: any[];
  PackingList: any[];
...
export default function CustomsExemptionRegistration(props: any): JSX.Element {
  ...
  const validationSchema = Yup.object().shape({
    ...
    CountryOfOrigin: Yup.string().required(t('Required')),
  ...
  const _getApplicationDetails = async () => {
    ...
    if (data?.payload.Data?.success) {
      ...
      const newMappedData: any = mapAddNewRawMaterial(updatedData);
  ...
  const onSubmit = (values: any) => {
    try {
      ...
      const body = transformNewRawMaterialData({
        ...values,
        userId: userId,
      ...
      dispatch(
        submitNewRawMaterial({body: {...body, languageId: language}}),
      ).then(result => {
            ...
            <View>
              ...
              <Input
                ...
                onChange={(val: any) => {
                  ...
                  setFieldValue('CountryOfOrigin', val.value);
                }}
                requiredStar
                required={touched.CountryOfOrigin && errors.CountryOfOrigin}
                error={touched.CountryOfOrigin && errors.CountryOfOrigin}
                disabled={isDisabled('CountryOfOrigin')}
              />
              <Input
                ...
                onChangeText={(text: any) =>
                  setFieldValue('localGulfOriginForeign', text)
                }
                disabled
              />
              <RawMaterialsDetails isDisabled={isDisabled('RawMaterial')} />
              <StepIndicator
                stepNumber={3}
              ...
              <Input
                textInput
                label={t('IL.Total')}
                value={String(values.ServiceFees * values.RawMaterials.length)}
                disabled
              />
              <StepIndicator
                stepNumber={4}
              ...
              />
              <FileUpload
                title={t('IL.CE.InvoiceAttachment')}
